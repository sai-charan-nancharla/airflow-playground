...
# schedule = @daily,monthly,weekly is preset from airflow to run the DAG every day/houlry/mothly
# is_paused_upon_creation is set to False to make the DAG active upon creation unless paused.

@dag(
    dag_id="scheduling_preset_dag",
    start_date=datetime(
        year=2026,
        month=6,
        day=1,
